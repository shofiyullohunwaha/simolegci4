<?php
    ...
    if(is_array($dtlogin)){
        if(count($dtlogin)>0){
            foreach($dtlogin as $mm){
                $namaprofil = $mm->nama;
                $levelprofil = $mm->level;
            }
        }
    }
?>
...
<head>
    ...
    <link rel="stylesheet" href="<?= BASEURLKU; ?>writable/assets/css/custom.css">
    ...
    <script src="<?= BASEURLKU; ?>writable/assets/js/plugin/highcharts.js"></script>
    <script src="<?= BASEURLKU; ?>writable/assets/js/plugin/tinymce/tinymce.min.js"></script>
        ...
        <div class="main-header">
            <div class="logo-header" data-background-color="blue2">
                <a href="javascript:void(0)" class="logo">
                    <img src="<?= BASEURLKU; ?>writable/assets/img/logo.png" alt="navbar brand" class="navbar-brand"
                        style="width: 45px;">
                    <font style="font-size: 23px; color: white; vertical-align: middle;"> Metrologi </font>
                </a>
                <br>
                    ...
                    <li class="nav-item dropdown hidden-caret">
                        ...
                        <ul class="dropdown-menu dropdown-user animated fadeIn">
                            <div class="dropdown-user-scroll scrollbar-outer">
                                <li>
                                    <div class="user-box">
                                        <div class="avatar-lg"><img src="<?= $linkfoto; ?>" alt="image profile"
                                                class="avatar-img rounded"></div>
                                        <div class="u-text">
                                            <h4 id="bloknama"><?= $namaprofil; ?></h4>
                                            <p class="text-muted" id="bloklevel"><?= $levelprofil; ?></p>
                                        </div>
                                    </div>
                                </li>
                                <li>
                                    ...
                                    <a class="dropdown-item" href="javascript:void(0)" data-toggle="modal"
                                        data-target="#modalprofil" data-backdrop="static" data-keyboard="false">Ubah
                                        Profil</a>
                                    <a class="dropdown-item" href="javascript:void(0)" onclick="hapusfotoprofil()">Hapus
                                        Foto Profil</a>
                                    <a class="dropdown-item" href="javascript:void(0)" data-toggle="modal"
                                        data-target="#modalpassword" data-backdrop="static" data-keyboard="false">Ganti
                                        Password</a>
            ...
            <div class="sidebar-wrapper scrollbar scrollbar-inner">
                <div class="sidebar-content">
                    <ul class="nav nav-primary">
                        <li class="nav-item">
                            <a href="<?= BASEURLKU; ?>beranda"><i class="icon-arrow-left"></i>
                                <p>Sistem</p>
                            </a>
                        </li>
                        <li class="nav-item" id="liberanda">
                            <a href="" id="mnberanda"><i class="icon-home"></i>
                                <p>Beranda</p>
                            </a>
                        </li>
                        <?php
                            if(is_array($dtmenu)){
								foreach($dtmenu as $x){
									...
									$icmenu = $x->icon_menu;
									echo '
										<li class="nav-item submenu" id="li'.$idm.'">
											<a data-toggle="collapse" href="#mn'.$idm.'">
												<i class="'.$icmenu.'"></i>
												<p>'.$menu.'</p>
												<span class="caret"></span>
											</a>
											<div class="collapse" id="mn'.$idm.'">
												...
												if(is_array($dtform)){
													foreach($dtform as $y){
														...
														if($idme == $idm){
                                                            if($tampil == "Y"){
                                                                echo '<li class="" id="mnx'.$iform.'"><a href="'.BASEURLKU.$iform.'"><span><i class="'.$icform.'"></i> '.$form.'</span></a></li>';
                                                            }
														}
													}
												}
            ...
            <footer class="footer">
                <div class="container-fluid">
                    <div class="copyright ml-auto">
                        <?= date("Y"); ?>, Development By Andrianyah pk
                    </div>
                </div>
            </footer>
                        ...
                        <div class="modal-body">
                            <div class="row">
                                <div class="form-group col-12 jedaobyek">
                                    ...
                                    <input type="password" class="form-control formps" id="txtpl"
                                        placeholder="Password Lama Anda">
                                </div>
                                <div class="form-group col-12 jedaobyek">
                                    <label>Password Baru</label>
                                    <input type="password" class="form-control formps" id="txtpb"
                                        placeholder="Password Baru Anda">
                                </div>
                                <div class="form-group col-12 jedaobyek">
                                    <label>Konfirmasi Password Baru</label>
                                    <input type="password" class="form-control formps" id="txtpk"
                                        placeholder="Konfirmasi Password Baru Anda">
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" id="btnbatalpassword"
                                class="btn btn-secondary waves-effect waves-light" data-dismiss="modal">Batal</button>
                            <button type="button" id="btnresetpassword" class="btn btn-danger waves-effect waves-light"
                                onclick="resetpassword()">Reset</button>
                            <button type="button" id="btngantipassword" class="btn btn-primary waves-effect waves-light"
                                onclick="gantipassword()">Update</button>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="row">
                                <div class="form-group col-12 jedaobyek">
                                    <label>Nama</label>
                                    <input type="text" class="form-control formprofil khusus_abjad" id="txtnamaprofil"
                                        placeholder="Masukkan Nama Anda" value="<?= $namaprofil; ?>">
                                </div>
                                <div class="form-group col-12 jedaobyek">
                                    <label>Gambar Profil (*.jpg)</label>
                                    <input type="file" class="form-control formprofil" id="txtfileprofil"
                                        accept="image/jpeg">
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" id="btnbatalprofil" class="btn btn-secondary waves-effect waves-light"
                                data-dismiss="modal">Batal</button>
                            <button type="button" id="btnresetprofil" class="btn btn-danger waves-effect waves-light"
                                onclick="resetprofil()">Reset</button>
                            <button type="button" id="btnupdateprofil" class="btn btn-primary waves-effect waves-light"
                                onclick="updateprofil()">Update</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
$("#mnberanda").attr("href", "<?= BASEURLKU.$ids; ?>");
$(".select2").select2();
$("#li<?= $idmskr; ?>").addClass("active");
$("#mnx<?= ucfirst($idf); ?>").addClass("active");
...
function updateprofil() {
    ...
    if (nama == "") {
        swal({
            title: 'Gagal',
            text: 'Ada Isian Nama Profil Masih Kosong !',
            icon: 'error',
        });
    ...
    if (files.length != 0) {
        ...
        if (x[jmlx - 1] != "jpg" && x[jmlx - 1] != "JPG") {
            swal({
                title: 'Update Gagal',
                text: 'File Harus Ber-ekstensi jpg !',
                icon: 'error'
            });
            $("#btnbatalprofil").attr("disabled", false);
            $("#btnresetprofil").attr("disabled", false);
