...
class Kb002 extends BaseController{
    protected $hasilc;
    protected $aksesc;
    public function __construct(){
        $h = $this->CekLogin2("form", "kb002");
        $this->aksesc = $h[2];
        $this->hasilc = $h;
    }
    ...
    public function json(){
        ...
        foreach ($dt as $k){
            ...
            if(strpos($this->aksesc, "002") !== false){
                $tomboledit = sprintf("<button type='button' class='btn btn-icon btn-round btn-primary btn-sm mr-1 mb-1' data-id_kab='%s' onclick='filterin(this)'><i class='icon-pencil'></i></button>", $id_kab);
            }
            if(strpos($this->aksesc, "003") !== false) {
                $tombolhapus = sprintf("<button type='button' class='btn btn-icon btn-round btn-danger btn-sm mr-1 mb-1' data-id_kab='%s' onclick='hapus(this)'><i class='icon-trash'></i></button>", $id_kab);
            }
           
            // Add the button HTML and other data to the row
            $dtisi .= sprintf('["%s","%s","%s"],', $tomboledit.$tombolhapus, $id_kab, $nama);
        }
    ...
    public function filter() {
        $status = false;
        $id_kab = antiSQLi($this->request->getVar("id_kab"));
        $nama = "";
        $dt = $this->mkb002->Filter($id_kab);
    ...
    public function add(){
        if(strpos($this->aksesc, "001") !== false){
            ...
            $hasilcek = true;
            $cek = $this->mkb002->Filter($id_kab);

            if(is_array($cek)){if(count($cek)>0){$hasilcek = false;}}
            if($hasilcek){
                
                $opr = $this->mkb002->Add($id_kab, $nama,  IdLogin());
                if($opr == "1"){
                    $ket = "ID: $id_kab,\nNama: $nama,";
    ...
    public function update(){
        if(strpos($this->aksesc, "002") !== false){
            ...
            $opr = $this->mkb002->Updatex($id_kab,$nama,IdLogin());
            if($opr == "1"){
                $ket = "ID: $id_kab,\nNama: $nama,";
    ...
    public function delete(){
        if(strpos($this->aksesc, "003") !== false){
            ...
            if($detail){
                $opr = $this->mkb002->Deletex($id_kab);
                if($opr == "1"){
                    $ket = "ID: $id_kab,\nNama: $nama,";
